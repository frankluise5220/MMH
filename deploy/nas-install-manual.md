...
>
> 卸载时选什么：从飞牛系统应用中心手动卸载 MMH 时，会弹出「是否删除用户数据」选择。默认「保留用户数据（推荐）」——只卸载软件，账簿数据留在原处，重新安装后可继续使用；只有明确选择「删除用户数据」才会清空数据目录（删除前仍会自动备份一份到数据目录外的 `mmh-upgrade-backups`）。软仓客户端的更新走静默卸载重装，不会弹这个向导，更新始终保留数据。
