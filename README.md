...
基金、存款、贷款这些"条线"大家都有，MMH 更在意的是每天记账和对账时手是不是顺：

- **表格按你的习惯重排和定制**：列宽直接拖、列顺序可以调、点表头排序，表头还有统一的列设置和下拉筛选；这些偏好存下来，下次打开还是你排好的样子。账户、分类、标签也能在侧边栏和下拉里拖拽排序。
- **批量改账**：多选流水后批量改分类、换账户、加标签、删除，误操作可撤销；外部账单也是先预览、确认后批量入账，整批同样可撤销。
- **计划任务自动落账**：定投、还款、转账、缴费、存款取息、债券付息都收进计划任务中心，到点自动生成流水——不用每月手动抄一遍，也不怕漏记或重复记。
- **录入时少点几下**：「再记一笔」保留当前模式和常用默认值；可搜索下拉里能边选边新建账户、分类、标签；日期、金额、往来对象按历史和现有账户智能补全。
- **改了哪里只刷哪里**：保存后只刷新受影响的记录和合计，不整页重绘；编辑、再记一笔都留在原上下文，数字表紧凑、稳定、能扫。
- **一家人共用一本账**：账户所有人和家庭成员是同一批人，一边改名另一边同步；支出可以选家庭成员、机构、往来对象，往来款能算清谁欠谁。
