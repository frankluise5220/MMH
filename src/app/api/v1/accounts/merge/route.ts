...
export async function POST(req: NextRequest) {
  try {
    ...
    if (keepId === mergeId) {
      return NextResponse.json({ ok: false, code: "SAME_ACCOUNT", error: "Cannot merge an account into itself" }, { status: 400 });
    }
    ...
    if (!keep || !merged) {
      return NextResponse.json({ ok: false, code: "ACCOUNT_NOT_FOUND", error: "Account not found" }, { status: 404 });
    }
    // Both accounts must belong to the caller's household.
    if (keep.householdId !== householdId || merged.householdId !== householdId) {
      return NextResponse.json({ ok: false, code: "FORBIDDEN", error: "Account belongs to another household" }, { status: 403 });
    }
    ...
    if (keepKind !== mergeKind) {
      return NextResponse.json({ ok: false, code: "KIND_MISMATCH", error: "Only accounts of the same kind can be merged" }, { status: 400 });
    }
    ...
    if ((keep.institutionId ?? "") !== (merged.institutionId ?? "")) {
      return NextResponse.json({ ok: false, code: "INSTITUTION_MISMATCH", error: "Institutions differ" }, { status: 400 });
    }
    if (keep.currency !== merged.currency) {
      return NextResponse.json({ ok: false, code: "CURRENCY_MISMATCH", error: "Currencies differ" }, { status: 400 });
    }
    ...
    return NextResponse.json({ ok: false, code: "INTERNAL_ERROR", error: e instanceof Error ? e.message : "Merge failed" }, { status: 500 });
  }
}
