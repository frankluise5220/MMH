...
export async function POST(req: NextRequest) {
  try {
    ...
    if (endDate && !parsedEndDate) {
      return NextResponse.json({ ok: false, code: "INVALID_END_DATE", error: "结束日期不正确" }, { status: 400 });
    }
  ...
  try {
    ...
    if (existingTaskType === "loan_repayment" && amount != null) {
      ...
      if (requestedAmount == null || Math.abs(requestedAmount - currentAmount) > 0.001) {
        return NextResponse.json({ ok: false, code: "LOAN_REPAYMENT_AMOUNT_LOCKED", error: "还款金额由还款计划生成，不能在计划任务中修改；如需调整请在贷款模块操作。" }, { status: 403 });
      }
    }
    ...
    if (parsedNextRunDate && parsedNextRunDate < nextStoredStartDate) {
      return NextResponse.json({ ok: false, code: "NEXT_RUN_DATE_BEFORE_START_DATE", error: "下次执行日不能早于生效日期" }, { status: 400 });
    }
    // 贷款还款计划已按期生成交易记录（带 regularInvestPlanId / source 标识），
    // 下次执行日不允许回拨到已生成的最近一期之前，否则自动执行会停在旧日期。
    if (existingTaskType === "loan_repayment" && parsedNextRunDate && existing.lastRunDate && parsedNextRunDate.getTime() <= existing.lastRunDate.getTime()) {
      return NextResponse.json({ ok: false, code: "NEXT_RUN_DATE_BEFORE_LAST_RUN", error: `下次执行日不能早于最近一期已生成的记录（${existing.lastRunDate.toISOString().slice(0, 10)}）；如需重新生成历史记录，请在贷款模块使用重算。` }, { status: 400 });
    }
