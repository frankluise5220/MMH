...
export const SYSTEM_BOND_BUY_CATEGORY = "城投债买入";
export const SYSTEM_BOND_REDEEM_CATEGORY = "城投债赎回";
export const SYSTEM_BOND_INTEREST_CATEGORY = "城投债利息";
export const SYSTEM_BOND_WRITE_OFF_CATEGORY = "城投债核销";
...
export const SYSTEM_WEALTH_INVESTMENT_ACTION_CATEGORIES = [
  ...
  SYSTEM_BOND_BUY_CATEGORY,
  SYSTEM_BOND_REDEEM_CATEGORY,
  SYSTEM_BOND_INTEREST_CATEGORY,
  SYSTEM_BOND_WRITE_OFF_CATEGORY,
] as const;
...
export function getInvestmentCategoryName(entry: {
  ...
  if (productType === "wealth" || productType === "bond") {
    // 城投债 = 理财下的 bond 产品类型，分类文案独立成组（利息/核销口径对债更准确）
    const isBond = productType === "bond";
    if (subtype === "write_off") return isBond ? SYSTEM_BOND_WRITE_OFF_CATEGORY : SYSTEM_WEALTH_WRITE_OFF_CATEGORY;
    if (subtype === "redeem" || subtype === "switch_out") return isBond ? SYSTEM_BOND_REDEEM_CATEGORY : SYSTEM_WEALTH_REDEEM_CATEGORY;
    if (subtype === "dividend_cash") return isBond ? SYSTEM_BOND_INTEREST_CATEGORY : SYSTEM_WEALTH_DIVIDEND_CATEGORY;
    return isBond ? SYSTEM_BOND_BUY_CATEGORY : SYSTEM_WEALTH_BUY_CATEGORY;
  }
