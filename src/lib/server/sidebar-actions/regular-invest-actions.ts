...
async function updateRegularInvest(formData: FormData) {
  ...
  if (existingTaskType === "loan_repayment" && formData.has("amount")) {
    ...
    if (requestedAmount == null || Math.abs(requestedAmount - currentAmount) > 0.001) {
      return { ok: false as const, code: "LOAN_REPAYMENT_AMOUNT_LOCKED", error: "还款金额由还款计划生成，不能在计划任务中修改；如需调整请在贷款模块操作。" };
    }
  }
  const isFundTask = taskType === "fund_regular_invest";
  const isOrdinaryTask = taskType === "income" || taskType === "expense";
  ...
  const nextStoredStartDate = parsedStartDate
    ...
    : plan.startDate;
  if (parsedNextRunDate && parsedNextRunDate < nextStoredStartDate) return { ok: false as const, code: "NEXT_RUN_DATE_BEFORE_START_DATE", error: "下次执行日不能早于生效日期" };
  // 贷款还款计划已按期生成交易记录，下次执行日不允许回拨到已生成的最近一期之前。
  if (existingTaskType === "loan_repayment" && parsedNextRunDate && plan.lastRunDate && parsedNextRunDate.getTime() <= plan.lastRunDate.getTime()) {
    return { ok: false as const, code: "NEXT_RUN_DATE_BEFORE_LAST_RUN", error: `下次执行日不能早于最近一期已生成的记录（${plan.lastRunDate.toISOString().slice(0, 10)}）；如需重新生成历史记录，请在贷款模块使用重算。` };
  }
...
async function deleteRegularInvest(formData: FormData) {
  ...
  // regular-invest API instead, because their removal touches the real source
  // of truth (the loan / the deposit lot / the bond) and the row is re-created
  // by the startup self-heal when the source is still alive. The client routes
  // these to `/api/v1/regular-invest?id=...` and lets the user choose between
  // "delete the task only, keep the records" (keepSource=1) and deleting both
  // (cascadeSource=1 / cascadeLoan=1).
