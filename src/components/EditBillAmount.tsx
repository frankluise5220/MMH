...
export default function EditBillAmount({
  ...
  const [val, setVal] = useState(String(currentAmount.toFixed(2)));
  ...
  if (editing) {
    return (
      <div className="flex items-center gap-1">
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter") {
              ...
              if (Number.isFinite(num)) save(num);
            }
        ...
        {errMsg && <span className="text-[10px] text-red-500">{errMsg}</span>}
      </div>
    );
  }
  ...
  return (
    <span
      ...
      onClick={() => { setVal(String(currentAmount.toFixed(2))); setEditing(true); }}
      title="点击修改账单金额"
    >
      {formatMoney(currentAmount)}
      {hasOverride ? (
        <Unlock className="h-3.5 w-3.5 shrink-0 cursor-pointer text-orange-400 hover:text-orange-600" onClick={(e) => { e.stopPropagation(); reset(); }} aria-label="解锁，恢复自动计算" />
      ) : null}
    </span>
  );
}