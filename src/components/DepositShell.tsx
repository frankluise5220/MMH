...
export function DepositShell({
  ...
  const entryColumns = useMemo<AdvancedDataTableColumn<DepositEntry>[]>(() => [
    ...
    { key: "product", label: t("depositShell.colProduct"), width: 190, minWidth: 120, filterKind: "text", filterText: (entry) => entry.fundName, filterSearchText: (entry) => entry.fundName, sortValue: (entry) => entry.fundName, render: (entry) => <span className="truncate text-slate-700" title={entry.fundName}>{entry.fundName || "-"}</span> },
    ...
    { key: "cashAccount", label: t("depositShell.colCashAccount"), width: 150, minWidth: 100, hideable: true, filterKind: "text", filterText: (entry) => entry.cashAccountLabel, filterSearchText: (entry) => entry.cashAccountLabel, sortValue: (entry) => entry.cashAccountLabel, render: (entry) => <span className="truncate text-slate-600" title={entry.cashAccountLabel}>{entry.cashAccountLabel || "-"}</span> },
    { key: "note", label: t("detail.column.remark"), width: 240, minWidth: 120, hideable: true, filterKind: "text", filterText: (entry) => entry.note, filterSearchText: (entry) => entry.note, sortValue: (entry) => entry.note, render: (entry) => <span className="block truncate text-slate-600" title={entry.note}>{entry.note || "-"}</span> },
    {
      key: "amount",
