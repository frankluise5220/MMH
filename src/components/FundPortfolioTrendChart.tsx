...
type FundTrendPoint = {
  ...
  netFlow: number;
  /** Cash dividend of the month (¥); does not reduce cost basis */
  dividendCash?: number;
...
type ApiResponse = {
  /** API responses carry ok; RSC prefetch (direct value from loadFundPortfolioTrendData) does not */
  ok?: boolean;
...
type ChartRow = FundTrendPoint & {
  /** 1 + cumulative portfolio return; shares the pct axis with the benchmark (value 1 = 0%) */
  portfolioPct: number | null;
  /** Normalized benchmark NAV (first month = 1) */
  benchmarkPct: number | null;
  /** No transaction in this month (backend emptyMonths) */
  isEmpty: boolean;
};
...
export default function FundPortfolioTrendChart({ initialData, refreshKey }: Props) {
  ...
  const compactTick = (v: number) => {
    if (Math.abs(v) >= 10000) return `${(v / 10000).toFixed(1)}${t("stats.compact10k")}`;
  ...
  // RSC-prefetched initialData may lack benchmark (empty BenchmarkCache; SSR does
  // not hit external APIs). When positions exist but benchmark is empty, fetch
  // once on mount (the API route refreshes the benchmark cache as a side effect).
  useEffect(() => {
