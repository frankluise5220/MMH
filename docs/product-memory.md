...
- Account notes are user-owned freeform text. Store, search, sync, export, restore, and edit them as ordinary account remarks; do not restrict, classify, warn, or judge how the user chooses to use that field.
...
- 股票业务应新增独立业务表和统一服务模块，例如 `StockSecurity`、`StockTransaction`、`StockHolding`、`StockPriceCache`、`StockFeeRule` 和 `src/lib/stock/**`；股票手续费规则可以借鉴现有按账户、产品代码、费用类型和生效日期查询的模式，但代码和数据模型必须归入 stock 域，支持佣金、印花税、过户费、经手费、监管费、最低收费、买卖方向和券商账户差异。
...
- 胶囊菜单必须提供独立“股票”入口。第一次股票买入时若当前账簿没有股票账户，交易弹窗自动建立 `investProductType = "stock"` 的股票账户；创建股票账户时，如果已选择证券机构，服务端必须同时确保该机构、该所有人、该币种下存在一个现金/钱包类“证券资金账户”，不存在则自动建立。已有一个或多个股票账户时必须让用户选择账户。股票买入、卖出、分红和税费调整的资金来源/去向默认使用同券商机构下的证券资金账户，没有单独资金账户时才兼容退回股票账户自身现金。银行与券商之间的入金/出金作为单独“银证转账”普通转账处理，目标/来源是证券资金账户；同一证券公司名下的股票账户和基金账户可以通过 `cashAccountId` 共用这笔可用资金，但股票业务表、API 和 UI 仍全部归入 `stock` 域，不复用基金字段。股票账户页应分开展示证券资金账户现金、持仓市值、总资产和浮动盈亏，不能把未买入的现金混进 `StockHolding`。
- 股票交易弹窗里的买入、卖出、分红、送股、拆股、并股、费用调整、税费调整不是同一种录入界面套不同标题；每个动作必须只显示它需要的字段。买卖显示数量、价格、成交金额、净额和明细手续费；现金分红显示分红金额、净到账和扣税/费用；送股、拆股、并股只显示股数变化；费用/税费调整只显示现金扣款金额。
