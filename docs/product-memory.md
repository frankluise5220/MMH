...
- Feiniu/fnOS distribution has exactly one user-facing app package, `mmh.fpk`. It may use the SQLite/Next standalone runtime internally, but do not publish or document a separate `mmh-native.fpk`; ordinary NAS installation remains Docker-based. The in-app system update page must distinguish fnOS from Docker/source deployments: fnOS updates are managed by the Feiniu app center or by installing a newer `mmh.fpk`, not by Git pull or Docker image update.
...
- Database settings should expose only the JSON household restore package for backup/recovery; do not expose an XLSX table backup or Prisma Studio entry as normal user-facing settings.
- Access whitelist entries are normalized hostnames, and the proxy must enforce them before public/login routes. User-facing entry should accept a hostname, IP, or URL with a port but store only the hostname.
...
- When credit card statement import parses a statement amount such as 本期应还、本期应缴余额、本期账单金额, New Balance, or Total Due, save it as a `BillOverride` for that statement month so the issued bill amount is locked to the bank statement value. Email bill import previews must visibly show the detected statement amount before import and the locked account/month/amount after import completes.
...
- Each household has one current/base display currency. Account records and transaction records keep their own original currency, while sidebar totals, net worth, and cross-account summaries convert foreign-currency balances into the household base currency through cached `FxRate` rows.
...
- Display settings should expose the household current/base currency selector and an explicit "获取汇率" action, but should not show a rate table or manual rate-entry table. Cached rates remain a calculation/data concern and may be managed by explicit refresh actions or workflows.
- Missing foreign-exchange rates must be explicit in the UI/API. Do not silently treat a missing rate as 1:1; if the rate is missing, omit that foreign-currency amount from converted totals and show which currency still needs a rate. Rate fetching should write a cache row and normal page refresh should avoid slow external FX calls unless the flow explicitly refreshes rates.
...
- Fund holding cost basis uses the same net confirmed buy amount (`gross buy amount - linked refund amount - buy fee`). Redemption realized profit uses the net arrival amount; if no separate arrival amount is stored, `TxRecord.amount` is already the cash-side net receipt and must not have the redemption fee deducted again during holding recalculation.
...
- On app startup, the system should run a lightweight background check after login: execute due scheduled tasks, refresh latest NAV for all current fund-like holdings, then fill due pending fund buy rows whose NAV or units are missing. This startup check must run from server-side database queries, not by loading every fund page in the client. Held fund NAV refresh must be based on current `FundHolding` rows, not only regular-invest plans or pending buy rows; pending buy unit calculation must use the same net confirmed amount rule including linked refunds.
