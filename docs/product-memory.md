...
- 股票交易弹窗里的买入、卖出、分红和低频股本变动不是同一种录入界面套不同标题；每个动作必须只显示它需要的字段。买卖高频窗口显示股票账户、证券资金账户、股票代码/自动查询名称、交易日期、数量、价格、成交金额和只读费用预估，数量与成交价格放在同一行，成交金额由数量 × 成交价格自动得出；费用区只直接露出费用合计、成交金额和预计应付/到账，三个金额框必须等宽、等高、同层级，并在标签或数值中明确显示币种（A 股为 CNY），佣金、印花税、过户费、经手费、监管费、其他费用只放在费用合计的 hover 明细里。股票市场在窗口中优先由股票代码推断；费用金额按账户费率和市场默认规则自动计算，用户不在交易窗口手工改费用；窗口可提供“获取新费率”按钮刷新公开市场规则并重算预估，但按钮必须独立于三个金额框，不能改变三者布局。买入的预计应付 = 成交金额 + 费用合计，并从证券资金账户扣除；卖出的预计到账 = 成交金额 - 费用合计，并进入证券资金账户。费率设置仍放在股票视图持仓列表表头的“账户费率”入口中，作为股票账户级/市场级设置维护。送股、拆股、并股不是常规高频操作，不占用三个交易入口，应合并为一个“股本变动”入口，在入口内选择具体变动类型并只填写股数变化。费用调整、税费调整这类影响整体股票账户现金/规则的动作不出现在股票交易弹窗，放在股票视图持仓列表表头入口处理。交易弹窗不显示“记录证券买入”“银证转账进这里”“保存后……”等说明文案，底部只保留一个右下角“保存”按钮；现金分红显示分红金额和可选净到账。
