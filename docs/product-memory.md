...
- Feiniu/fnOS distribution has exactly one user-facing app package, `mmh.fpk`. It may use the SQLite/Next standalone runtime internally, but do not publish or document a separate `mmh-native.fpk`; ordinary NAS installation remains Docker-based.
...
- Display settings should expose the household current/base currency selector, a rate table, a manual "获取汇率" action, and manual rate entry fields for source currency, rate, and rate date. Normal page loads may read cached rates, but external rate fetching should happen only when the user explicitly refreshes rates or a workflow explicitly asks to fetch them.
