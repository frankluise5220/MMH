...
- Feiniu/fnOS uses SQLite and therefore has no separate database connection password. Destructive system actions (system initialization, deleting ledgers) verify the password of the currently logged-in admin user; they do not use a deployment-level system/database password. The fnOS install/config wizard must not ask for a separate system password; a legacy `MMH_SYSTEM_PASSWORD` may still be generated internally and persisted for compatibility only.
...
- The verified FN soft-store/custom-source path is the public GitHub Raw repository source `https://raw.githubusercontent.com/frankluise5220/MMH/main/deploy/fnos/repository`, which exposes the `/api/apps` source file and points downloads to GitHub Releases. Do not confuse this with the optional self-hosted source service on port `5660`; users should use the GitHub source unless a separate public service has been deliberately deployed.
...
- 股票视图的持仓表和交易明细表必须使用与基金视图相同的 `AdvancedDataTable` 组件能力：列宽记忆、表头排序、表头筛选、隐藏列、复选、批量删除、行删除、汇总行。持仓表点击行加载交易明细，明细表按账户+股票 `resetKey` 隔离筛选状态。股票交易删除走 `DELETE /api/v1/stocks/transactions?id=` 软删除并重算持仓。
...
- 股票交易弹窗里的买入、卖出、分红和低频股本变动不是同一种录入界面套不同标题；每个动作必须只显示它需要的字段。买卖高频窗口显示股票账户、证券资金账户、股票代码/自动查询名称、交易日期、数量、价格、成交金额和只读费用预估，数量与成交价格放在同一行，成交金额由数量 × 成交价格自动得出；费用区只直接露出费用合计、成交金额和预计应付/到账，三个金额框必须等宽、等高、同层级，并在标签或数值中明确显示币种（A 股为 CNY），佣金、印花税、过户费、经手费、证管费、其他费用只放在费用合计的 hover 明细里。股票市场在窗口中优先由股票代码推断；费用金额按账户费率和市场默认规则自动计算，用户不在交易窗口手工改费用；窗口可提供“获取新费率”按钮刷新公开市场规则并重算预估，但按钮必须独立于三个金额框，不能改变三者布局。买入的预计应付 = 成交金额 + 费用合计，并从证券资金账户扣除；卖出的预计到账 = 成交金额 - 费用合计，并进入证券资金账户。费率设置仍放在股票视图持仓列表表头的“账户费率”入口中，作为股票账户级/市场级设置维护。送股、拆股、并股不是常规高频操作，不占用三个交易入口，应合并为一个“股本变动”入口，在入口内选择具体变动类型并只填写股数变化。费用调整、税费调整这类影响整体股票账户现金/规则的动作不出现在股票交易弹窗，放在股票视图持仓列表表头入口处理。交易弹窗不显示“记录证券买入”“银证转账进这里”“保存后……”等说明文案，底部只保留一个右下角“保存”按钮；现金分红显示分红金额和可选净到账。
...
- 股票名称一律以本地已保存数据为准：`StockSecurity`、`StockHolding`、`StockTransaction` 里已有的名称直接使用，交易窗口输入股票代码时只查本地，不触发外部股票查询 API。只有首次买入且本地完全没有该股票名称时，服务端 `resolveOrCreateStockSecurity` 才调用外部查询补全名称并缓存；`GET /api/v1/stocks/securities?lookup=1` 仅用于显式请求补全名称。
...
- 房产购入、装修投入和出售写入 `PropertyAsset` / `PropertyTransaction`，需要资金账户时创建现金侧 `TxRecord` 并通过 `EntryBusinessLink.propertyTransactionId` 关联；购入和装修成本应包含交易金额、手续费和税费。手动更新房产市值只写 `PropertyValuation` 并更新 `PropertyAsset.marketValue`，不生成收入、支出、转账或投资现金流。出售时以净回收金额减累计成本计算已实现收益。
