...
- 数据目录必须持久化，升级不得删除用户的 SQLite 数据库文件。
...
- `scripts/build-fnos-package.cjs` 生成飞牛 FPK 工程，写入 `cmd/main`、manifest、图标、数据目录、Prisma runtime 和 SQLite 启动链。
