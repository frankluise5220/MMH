# 开发文档与同步更新说明

本文档说明开发过程中哪些内容应该写进哪些文档，以及代码变化时需要同步更新哪些资料。目标是让 Web、iOS、Android、NAS 部署和后续维护都能沿着同一套产品语义推进。

信用卡分期规则与计算统一由 `src/lib/credit/installment.ts` 负责；数据结构在 `CreditCardInstallmentPlan` 与 `TxRecord.installment*` 字段中。修改分期费率、账期或冲抵逻辑时，同步检查账单汇总、交易 API、移动同步和删除/恢复联动。

SQLite 恢复兼容建表时，`TxRecord` 只是 Prisma 模型名，实际表名是 `transactions`。房产交易的 `cashEntryId` 外键必须引用 `transactions(id)`；如果残留 `REFERENCES "TxRecord"("id")`，恢复会在 55%「清空当前账簿」失败。

## 文档分工

### `AGENTS.md`

用途：本地 agent 工作规则和长期产品/工程方向。

记录内容：

- 产品方向、数据层原则、API 原则、统一函数/模块方向。
- 反复确认过的长期决策，例如 Docker 更新方式、函数复用、局部刷新、Web/移动端分工。
- 不记录具体 host、账号、密码、SSH key、私有测试仓库地址等连接信息。

更新时机：

- 对话中形成了会长期影响后续开发的产品或工程决策。
- 发现 agent 经常重复犯同一类方向性错误。
- 文档分工、更新方式、API 规则、长期产品方向发生变化。

注意：

- `AGENTS.md` 是本地文件，不应作为公开安装说明。
- 私有地址、密码、测试密钥不能写入 `AGENTS.md`、公开文档或提交内容；需要临时使用时由当前会话或本机安全凭据提供。

### `docs/client-api.md`

用途：Web、iOS、Android 接入后端 API 的正式说明。

记录内容：

- 登录、服务器、账簿、用户、角色上下文。
- 通用返回格式、错误格式、日期/金额/分页/排序约定。
- 各模块 API：auth、overview、accounts、transactions、fund、settings、mobile sync。
- 每个接口的方法、路径、参数、返回结构和示例。

更新时机：

- 新增、删除或重命名 `/api/v1` 客户端接口。
- 修改接口参数、返回字段、错误格式、分页/排序规则。
- Web 和移动端都可能使用某个接口，或者移动端新增接入需求。
- 某个接口从 debug/internal 变为正式客户端契约。

### `docs/agent-api.md`

用途：Hermes、Codex、Claude 等外部 Agent 通过 HTTP 接入 MMH 后端的说明。

记录内容：

- 外部 Agent 可用的 Base URL、认证 header、通用返回格式。
- 可用的业务 API 和受控通用 DB API。
- 通用 DB API 的模型白名单、分页、账簿隔离和写入边界。
- 密钥、密码、token、邮箱账号、系统设置等敏感数据的访问禁区。

更新时机：

- 新增、删除或收紧外部 Agent 可调用接口。
- 修改 `/api/v1/db/models` 或 `/api/v1/db/data` 的认证、白名单、字段过滤、返回格式。
- 改变密钥存储、加密、脱敏、轮换或 Agent 权限策略。

### `docs/nas-install-manual.md`

用途：用户在普通 NAS、飞牛 fnOS 和群晖 DSM 上安装和更新 MMH 的说明。

记录内容：

- 飞牛 fnOS 的 FN 软仓安装/更新方式、手动 `.fpk` 安装方式和覆盖升级注意事项。
- 群晖 DSM 的手动 `.spk` 安装/更新方式和覆盖升级注意事项。
- 普通 NAS 的 Docker 图形界面安装方式和 Docker 命令行安装方式。
- 环境变量、端口、数据库、更新命令、常见问题。

更新时机：

- `docker-compose.yml`、`Dockerfile`、镜像名、端口、环境变量、飞牛 `.fpk` 命名、群晖 `.spk` 命名或软仓源地址发生变化。
- 安装流程、更新流程、系统更新页面能力发生变化。
- 发布方式从本地测试源切换或同步到 GitHub/GHCR。

### `docs/fnos-package-plan.md`

用途：飞牛 fnOS 应用包、速度优化和安全基线的落地计划。

记录内容：

- 飞牛包的 SQLite 原生运行方向，以及同一应用 ID 下的 x86/ARM64 架构化资产。
- 哪些内容放进包内，哪些内容必须继续由 GHCR 镜像提供。
- 飞牛包默认安全边界，例如不暴露数据库端口。
- 与速度、安全、系统更新页相关的后续工程清单。

更新时机：

- 新增或修改 `deploy/fnos/` 下的飞牛包素材。
- 飞牛 `.fpk` manifest、签名、目录结构、分发源字段得到确认。
- NAS/飞牛更新方式、镜像源、端口、数据目录策略发生变化。

### `docs/fnos-publish-submission.md`

用途：飞牛官方应用中心上架提交材料。

记录内容：

- 当前提交包路径、版本、SHA256、大小和公开下载地址。
- manifest 摘要、应用简介、权限说明和提交文案。
- 已验证项目、待人工补充的截图/录屏/联系方式。

更新时机：

- 生成新的飞牛官方提交包。
- GitHub Release 资产、应用版本、端口、权限说明或审核要求发生变化。

### `deploy/synology/README.md`

用途：群晖 DSM `.spk` 包的构建、发布和手动安装说明。

记录内容：

- 群晖包的 SQLite 原生运行方向，以及 x86_64/ARM64 架构化资产。
- `.spk` 正式产物和 `*-spk-source.tgz` 调试归档的区别。
- 打包命令、校验命令、GitHub Release workflow 和覆盖升级原则。

更新时机：

- 新增或修改 `deploy/synology/`、`.github/workflows/synology-*.yml` 或 `scripts/build-synology-*.cjs`。
- 群晖 `.spk` 的 INFO 字段、目录结构、启动脚本、端口或数据目录策略发生变化。
- NAS/飞牛/群晖更新方式、发布资产命名或系统更新页展示逻辑发生变化。

### `docs/minibill-speed-security-reference.md`

用途：记录 miniBill 对 MMH 在速度、安全和部署边界上的参考价值。

记录内容：

- 哪些工程原则值得吸收，例如轻运行时、小响应、清楚权限边界。
- 哪些内容不应照搬，例如 Go/SQLite 技术栈和简化业务模型。
- 速度、安全、飞牛包的 P0/P1/P2 落地清单。

更新时机：

- 因 miniBill 或类似项目形成新的性能、安全、发布包方向。
- 大列表分页、导入任务化、更新器安全边界、飞牛包发布链发生变化。

### `docs/security-hardening.md`

用途：远程登录、HTTPS、数据库端口、Prisma Studio、Cookie 和账簿隔离的安全边界说明。

记录内容：

- 哪些入口不能公开暴露。
- HTTPS、HSTS、Secure Cookie 的部署要求。
- 业务 API 必须使用当前 session 的账簿上下文进行过滤。
- 后续安全能力方向，例如登录限流、敏感字段加密、审计日志和会话管理。

更新时机：

- 登录/session Cookie、安全响应头、反向代理、数据库端口、Prisma Studio 暴露策略发生变化。
- 新增或收紧账簿隔离、审计、加密、限流等安全机制。

### `docs/android-release.md`

用途：Android 客户端打包、安装、发布说明（从 0.1.33 起 APK 随 `v0.1.x` Release 由 CI 自动构建，不再单独发 `android-v*`）。

记录内容：

- 构建命令、签名方式、安装方式。
- GitHub Secrets 签名配置（`ANDROID_KEYSTORE_BASE64` 等四个 secret）。
- Android 端服务器/账簿/用户登录配置。
- 与 Web API 的兼容要求。

更新时机：

- Android 工程结构、包名、签名、构建命令变化。
- `.github/workflows/android-release.yml` 或签名 Secrets 变化。
- Android 登录、设置、同步 API 使用方式变化。
- 移动端需要新的权限或系统能力。

### `docs/release-notes-template.md`

用途：GitHub Release 对外发布说明模板。

记录内容：

- 面向用户的更新摘要。
- 安装 / 更新影响。
- 兼容性说明。
- 已知问题。

更新时机：

- 发布方式变化。
- 用户需要在 GitHub 首页或 Release 页更直观看到更新内容。
- Android / NAS / Web 的发布说明结构需要统一。

补充规则：

- 小改动不强制写 Release 说明。
- 只有大改动、用户可感知变化、安装/更新变化、兼容性变化时，才优先补正式说明。

### `docs/edit-window-checklist.md`

用途：新增/编辑窗口的通用检查清单。

记录内容：

- 打开窗口时初始值是否来自数据库或统一派生数据。
- 保存时是否写回所有必要字段。
- 关闭、保存、再记一笔、局部刷新等交互是否符合预期。

更新时机：

- 新增通用弹窗行为。
- 重构新增/编辑窗口、SS 下拉、账户选择、基金选择等共享控件。
- 发现某类窗口反复漏字段或刷新错误。

### `src/components/TableColumnFilter.tsx`

用途：表格表头统一筛选下拉组件。
记录内容：
- 单击行即单选、确认并关闭的统一行为。
- 表头字段旁边的筛选入口样式。
- 哪些页面复用这一统一逻辑。
更新时机：
- 修改任何表头筛选交互时。
- 新页面要接入表头筛选时。

### `docs/product-memory.md`

用途：记录会影响多页面的长期产品设置和共享交互规则。
记录内容：
- 一处修改、多处调用的共享设置方向。
- 表头筛选、SS 下拉、保险、基金等反复确认过的产品规则。
- 表格列宽记忆、统一表头设置、统一筛选和统一排序等跨页面交互规则。
更新时机：
- 新增或改变会跨页面复用的设置、交互、命名规则时。

### `docs/check-investment-data.md`

用途：投资和基金数据核对说明。

记录内容：

- 持仓、净值、份额、成本、浮盈、历史收益的核对方法。
- 基金交易明细、确认日期、到账日期、手续费率、确认天数的检查方法。
- 股票持仓、收盘价缓存、交易明细的核对方法。

更新时机：

- 基金持仓重算逻辑变化。
- 净值缓存、外部净值查询、确认天数、费率逻辑变化。
- 投资账户视图或基金明细视图的关键字段变化。
- 股票持仓盈亏报表口径或报表页签变化。
- 股票持仓/交易明细表接入共享表格或缓存路径变化。

### `docs/fixed-asset-generalization.md`

用途：固定资产从「房产专用」泛化为「多类型固定资产」的设计与实施规则。

记录内容：

- 固定资产账户类型（fixed_asset）与底层 investment + property 的映射关系。
- 固定资产类型清单（房产、车辆、设备、家具、收藏品、其他；贵金属排除）。
- 账户与资产 1:N 关系、通用 FixedAsset 表与 assetType 字段、估值历史表。
- 估值与估值日期规则、市值变化曲线、交易规则、UI/UX 与 API 契约。
- 迁移步骤与已确认边界（含房产改名同步账户名逻辑的回退点）。

更新时机：

- 新增或调整固定资产类型、表结构、估值或交易规则时。
- 固定资产账户、资产、估值、交易相关 UI 或 API 契约变化时。

### `docs/architecture.md`

用途：系统分层架构与模块关系总览，面向非开发者也能看懂。

记录内容：

- 客户端、接口、业务逻辑、数据、部署五层结构。
- Web 工作台功能地图与关键数据流示例。

更新时机：

- 模块归属、数据流或部署结构发生重大变化。
- 新增主要业务模块时。

### `docs/i18n-migration.md`

用途：存量中文字面量迁移到 i18n 层的执行细则。

记录内容：

- 中文的迁移分类（UI 文案 / 注释 / 业务数据）与判定方法。
- i18n 键命名规范、每文件工作流、验证命令。
- 参照实现文件。

更新时机：

- i18n 键命名规范或迁移流程发生变化。
- 新增需要特殊处理的迁移模式时。

## 开发时的同步更新规则

### 文档写法

面向用户或接入方的文档要少解说、多说明。用户打开文档时，首先需要知道怎么操作、填什么、点哪里、看到什么结果、出错后怎么办。

优先写：

- 操作步骤。
- 可复制的命令。
- 字段名和示例值。
- 成功后的预期结果。
- 常见错误和处理办法。
- 哪些内容需要用户选择。

少写：

- 长篇背景。
- 开发者内部推理。
- 与用户操作无关的技术解释。
- 重复说明“为什么这样设计”。

只有当原因会影响用户选择、避免误操作、或帮助判断故障时，才写简短解释。

### 本仓库工具使用约束

为减少编码、引号、正则和终端乱码带来的噪音，源码检查和修改默认遵循下面规则：

- 搜索优先使用 `rg` / `rg --files`。
- 在 PowerShell 里搜索包含括号、引号、`|`、`$`、正则分组或中文的复杂内容时，优先使用 `npm run codex:rg -- <pattern> <path...>`，默认固定字符串搜索，避免 PowerShell 把参数拆坏。
- 查看或暂存带路径的 Git 变更时，优先使用 `npm run codex:git -- <git 参数...>`。例如 `npm run codex:git -- diff -- sidebar/page.tsx` 会把 `sidebar/...` 安全展开为 `src/app/(sidebar)/...`，避免 PowerShell 先解析括号。
- 不在 PowerShell 命令里裸写 `src/app/(sidebar)`；搜索、diff、add 等常见操作统一用 `sidebar/...` 路径别名。
- 需要临时查询 Prisma 数据时，不要写长 `node -e`；把查询写入被 Git 忽略的 `_tmp_query.cjs` / `.tmp-query.cjs`，再运行 `npm run codex:prisma -- _tmp_query.cjs`，由脚本统一加载 `.env`、Prisma adapter 和 `$disconnect`。
- 读取源码文件时优先显式按 UTF-8 读取。
- 修改源码文件时优先使用 `apply_patch`。
- 不把 PowerShell 终端输出当作中文内容是否正确的依据，中文以文件实际内容为准。
- 避免用复杂 PowerShell 正则、嵌套引号、长链式表达式去处理源码；优先拆成多个小而稳定的读取/搜索步骤。
- 如果某个文件已经出现乱码或混合编码风险，先清理编码问题，再继续做逻辑修改。

### API 变化

同时检查：

- `src/app/api/v1/**/route.ts` 顶部 JSDoc。
- `docs/client-api.md`。
- Web 调用处。
- Android/iOS 调用处或移动端同步接口。

需要记录：

- 方法和路径。
- 请求参数、请求体。
- 返回字段。
- 错误格式。
- 是否需要登录、账簿、角色上下文。
- 是否影响分页、排序、日期、金额格式。

### 数据库和业务计算变化

同时检查：

- Prisma schema 和迁移。
- 统一服务模块，例如 fund、account、transaction 相关模块。
- Web 展示页和编辑页。
- 移动端 summary/sync 数据。
- `docs/check-investment-data.md` 或相关核对文档。

重点：

- 不要让 Web 和移动端各算一套。
- 不要让列表值、顶部汇总值、左侧概览值来自不同口径。
- 涉及余额时，不使用收入减支出替代账户余额。

### UI/交互变化

同时检查：

- 共享组件或页面组件。
- `docs/edit-window-checklist.md`。
- Web 和移动端是否需要不同布局但同一业务含义。

重点：

- 保存后是否局部刷新。
- 再记一笔是否保留上下文。
- SS 下拉是否支持搜索和新增。
- 数字表格是否保持列宽、排序、颜色规则一致。

### NAS 安装和更新变化

同时检查：

- `Dockerfile`
- `docker-compose.yml`
- `.env.example`
- `docs/nas-install-manual.md`
- `scripts/install-windows.bat`
- 系统设置里的系统更新页面

重点：

- 普通更新目标是拉取预构建镜像，不在 NAS 或普通用户本机上常规构建。
- 本地测试源和 GitHub/GHCR 正式源应尽量只差 URL。
- 不把本地私有 NAS 地址、密码、测试 API 写入公开文档。

### 发布前检查

发布前至少确认：

- 对外发布必须同时覆盖 GitHub/GHCR 的 Docker 更新链路、飞牛 x86/ARM64 `.fpk` 更新链路、群晖 x86_64/ARM64 `.spk` 更新链路；Android APK 只在本次 release 包含 `android/` 目录变更时随 Release 自动构建。各路径版本号、构建时间和已验证结果一致后，才算发布完成。
- 对外版本号统一使用 `package.json` 中的 `0.1.x`。每次正式公开 push/release 前先运行 `npm run release:version`，只递增一级 patch，例如 `0.1.9` -> `0.1.10`；GitHub Release tag 使用 `v0.1.x`，GHCR 镜像 tag、应用版本、全部飞牛 `.fpk` manifest、飞牛源元数据和群晖 `.spk` INFO 都必须一致，不能再使用单独的 `-fnos` 或 `-synology` 版本号。
- 发布前运行 `npm run check:release-version`，确认 `package.json`、`package-lock.json`、Docker workflow、飞牛 workflow、群晖 workflow 和飞牛源元数据没有版本分叉。
- 新接口是否补了 route JSDoc 和 `docs/client-api.md`。
- 安装或更新方式是否影响 `docs/nas-install-manual.md` 或 `scripts/install-windows.bat`。
- Android 相关变更是否影响 `docs/android-release.md`。
- 基金/投资计算变化是否更新 `docs/check-investment-data.md`。
- 新增/编辑窗口行为变化是否更新 `docs/edit-window-checklist.md`。
- `.env.example` 是否只包含公开安全的示例值。

### 导入调试日志

开发环境的账单/基金导入会把结构化诊断日志写入：

```text
.codex-logs/batch-import.debug.log
```

每次导入使用一个预览窗口可见的 `traceId`，可按该编号串联账户加载、文件解析、工作表合并、模式判断、校验阻断、批量修改、导入请求和数据库事务结果。

查看最近日志：

```powershell
Get-Content .codex-logs/batch-import.debug.log -Tail 100
```

查找一次导入：

```powershell
Select-String -Path .codex-logs/batch-import.debug.log -Pattern "traceId"
```

日志只能记录数量、模式、耗时、文件扩展名、文件大小、HTTP 状态和错误类型等诊断字段，不记录账单原文、文件名、账户名称、备注、密钥或密码。该文件仅在 `NODE_ENV=development` 时写入，并由现有 `*.log` Git 忽略规则排除。

## 信用卡独立账单与合并账单

- `Account.creditBillMode` 区分 `separate`（独立账单）和 `consolidated`（同机构合并账单）。
- 合并范围由“同一账簿 + 同一机构 + consolidated”确定，不额外维护账单组表。
- 同机构信用卡共用账单日、还款日和账单模式；修改其中一张卡时同步其他卡。
- 新增信用卡选择机构后，从该机构已有信用卡带出账单日、还款日、额度和账单模式。额度仅作为新卡默认值，不覆盖已有卡额度。
- 合并模式下，交易仍属于具体信用卡；账单汇总、明细、还款、周期和手工覆盖金额按组内所有卡计算。
- `CreditCardCycle` 和 `BillOverride` 使用组内按 ID 排序后的第一张信用卡作为稳定存储账户；停用卡仍保留在组内以保证历史账单不变化。接口和页面必须通过统一 helper 解析，不能直接假设当前点中的卡就是存储账户。
- 手工修改某一期账单周期后，受影响的 `CreditCardCycle` 行使用 `lockSource=manual_cycle` 标记。交易、分期或金额重算可以更新这些行的金额与当前期状态，但不得删除这些行后按 `Account.billingDay` 重新生成周期日期。
- 当前不建模共享授信、总授信或银行额度占用规则；额度仍是每个账户自己的字段。


### `docs/product-todos.md`

用途：记录跨版本的长期产品待办清单，例如保险产品主档与保单拆分、保险账户外壳、定投统一为通用计划任务。

记录内容：

- 计划中的产品改造，例如保险产品主档（可复用产品定义）与保单/持仓拆分。
- 保险账户外壳与侧边栏导航调整。
- 定投扩展为通用计划任务（基金定投、还款、转账、保险缴费）。

更新时机：

- 新增大项产品改造计划。
- 某项计划的状态或实现方向发生变化。

> 注：本节原文曾在 2026-08 因编码问题损坏为 `?` 字符，已按当前产品方向重建。
