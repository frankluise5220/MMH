...
- [ ] 股票买入/卖出窗口应把数量和成交价格放在同一行；下一行只读展示费用合计、成交金额和预计应付/到账，三个金额框必须等宽、等高、同层级，并在标签或数值中明确显示币种（A 股为 CNY）。佣金、印花税、过户费、经手费、证管费、其他费用只通过费用合计的 hover 明细展示。费用金额来自股票账户费率和市场默认规则，不在交易窗口手工编辑。“获取新费率”按钮应独立于三个金额框，不得改变三者布局；点击后刷新公开市场默认费率并重算预估。保存时服务端再次按同一规则计算并写入 `StockTransaction`，买入资金侧现金流为成交金额 + 费用合计，卖出现金侧现金流为成交金额 - 费用合计
- [ ] 股票交易明细支持编辑：双击明细行或点击行内编辑按钮打开股票交易弹窗并预填动作、账户、日期、数量、价格、金额、备注；编辑态不读取费率、不显示费用预估区，保存走 `PATCH /api/v1/stocks/transactions?id=`，同步更新资金流水和 `StockHolding`，弹窗标题显示「编辑股票交易」
