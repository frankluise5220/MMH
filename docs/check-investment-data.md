...
- 基金确认天数没有单独配置时默认按 T+1；只有用户明确保存 T+0 时才按当天确认。T+N 自动推导必须按交易日计算，不能用自然日直接加天数，否则周末和节假日会让确认日偏早。
...
- 投资收益表的“缺净值”提示按基金净值公布模型判断，避免把“还没公布”误报成“缺失”：
  - QDII 类基金（名称含 QDII/标普/纳斯达克/纳指/道琼斯/美国/全球）按美股交易日历（`us_fund`）判断交易日，且只提示“净值公布历史中段”的缺口（基金已存在该日期之后的净值）；最近交易日净值尚未公布（T+1 滞后）或美股休市日不提示。
