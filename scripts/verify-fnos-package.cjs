...
expect(/20260911_rekey_debt_agreement_to_account/.test(buildScript) && /DROP TABLE IF EXISTS \\?"DebtAgreement\\?"/.test(buildScript) && /DebtAgreement_accountId_key/.test(buildScript), "fnOS SQLite migrations must rekey DebtAgreement to accountId (and tolerate a fresh-install table created by native-init) for existing databases.");
