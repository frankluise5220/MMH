...
expect(/ensure_out_of_tree_backup_root/.test(buildScript), "fnOS package must still create the sibling upgrade-backups directory with mmh ownership for upgrade_init / uninstall_init copies.");
expect(!/MMH delete-data aborted: no writable backup directory outside/.test(buildScript), "fnOS uninstall_callback must not require an out-of-tree backup before honoring wizard_delete_data=true.");
expect(!/MMH user data backed up to/.test(buildScript), "fnOS uninstall_callback must wipe immediately and must not write a pre-delete backup.");
...
expect(buildScript.includes('version: "20260903_z_repair_investment_business_sources"'), "fnOS SQLite migration set must recognize the Postgres-only investment source repair version, otherwise an upgraded device DB that recorded it fails the newer-schema guard and MMH refuses to start.");
expect(buildScript.includes('version: "20260905_add_property_mortgage_loan_account"'), "fnOS SQLite migration set must recognize the property-asset mortgage loan account version recorded by upgraded device databases.");
expect(/addColumnIfMissing\(db, "property_assets", "mortgageLoanAccountId", "TEXT"\)/.test(buildScript) && /property_assets_householdId_mortgageLoanAccountId_idx/.test(buildScript), "fnOS SQLite migrations must add property_assets.mortgageLoanAccountId and its index for existing databases.");
...
expect(/20260910_add_reimbursement_tables/.test(buildScript) && /CREATE TABLE IF NOT EXISTS "reimbursements"/.test(buildScript) && /CREATE TABLE IF NOT EXISTS "reimbursement_items"/.test(buildScript) && /reimbursement_items_txRecordId_key/.test(buildScript), "fnOS SQLite migrations must create reimbursement tables for existing databases.");
...
if (process.env.FNOS_VERIFY_BUILT_FPK === "1") {
  ...
  expect(/wizard_delete_data:-false/.test(uninstallCallbackScript), "Built fnOS uninstall_callback must keep data unless the wizard explicitly chose delete.");
  expect(!/upgrade-backups/.test(uninstallCallbackScript) && !/MMH delete-data aborted/.test(uninstallCallbackScript), "Built fnOS uninstall_callback must wipe immediately and must not require an out-of-tree backup.");
  expect(/MMH delete-data failed: user data still present/.test(uninstallCallbackScript) && /MMH user data deleted from/.test(uninstallCallbackScript), "Built fnOS uninstall_callback must fail if mmh.db remains after wizard_delete_data=true.");
